...
const AuthPage: React.FC = () => {
  ...
  const toggleTab = (loginTab: boolean) => {
    ...
    setError(null);
    setSuccessMsg(null);
    // Xóa sạch dữ liệu khi chuyển đổi form
    setLoginEmail('');
    setLoginPassword('');
    setSignupEmail('');
    setSignupPassword('');
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!validatePassword(signupPassword)) {
      setError('Mật khẩu yếu: Cần 8+ ký tự, bao gồm chữ HOA, chữ thường, số và ký tự đặc biệt.');
      return;
    }
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 bg-cover bg-center" style={{backgroundImage: "url('https://picsum.photos/1920/1080?food,delivery')"}}>
        ...
        <div className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 space-y-8 border border-white/20">
          ...
          {isLogin ? (
            <form className="space-y-5" onSubmit={handleLogin}>
              <div className="relative">
                ...
                <input type="email" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)} autoComplete="off" className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-orange-100 focus:bg-white outline-none transition-all text-sm font-medium" placeholder="Email" />
              </div>
              <div className="relative">
            ...
            <form className="space-y-5" onSubmit={handleSignup}>
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 mt-2 ml-1 leading-relaxed">Ít nhất 8 ký tự, 1 hoa, 1 thường, 1 số & 1 ký tự đặc biệt.</p>
              </div>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400"><LockIcon className="h-4 w-4" /></span>
          ...
          <div className="relative">
              ...
              <div className="relative flex justify-center text-[10px]"><span className="px-3 bg-white text-gray-400 font-bold uppercase tracking-widest">Hoặc đăng nhập qua</span></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
              <button className="flex justify-center items-center py-2.5 border border-gray-100 rounded-2xl bg-gray-50/50 hover:bg-white transition-all"><GoogleIcon className="w-5 h-5" /></button>
              <button className="flex justify-center items-center py-2.5 border border-gray-100 rounded-2xl bg-gray-50/50 hover:bg-white transition-all"><FacebookIcon className="w-5 h-5" /></button>
          </div>
        </div>
      </div>
